<!DOCTYPE html>
<html>
<head>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.4.0/p5.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700&display=swap" rel="stylesheet">

    <style>
        /* 确保画布与你的页面布局匹配 */
        body {
            margin: 0px;
        }

        /* 根据实际眼睛位置调整 */
        .container {
            width: 100%;
            min-height: 100vh;
            background-color: #8a8a8a;
        }

        body {
            margin: 0;
            font-family: Arial, sans-serif;
            background-color: #E5E5E5; /* 浅灰色背景，模仿幻灯片编辑器 */
            display: flex;
            justify-content: center;
            align-items: center;
            min-height: 100vh;
        }

        .eclipse-container {
        width: 100%;
        background-color: #FFFFFF;
        padding: 40px;
        box-sizing: border-box;
        font-family: 'Montserrat', Arial, sans-serif;
    }

    .eclipse-content {
        display: flex;
        justify-content: flex-start; /* 调整为左对齐，减少间距 */
        align-items: flex-start;
        gap: 20px; /* 添加固定间距，更贴近图片 */
    }

    .eclipse-text {
        width: 50%;
    }

    .eclipse-text h1 {
        font-size: 1.5rem;
        color: #000000;
        margin: 0;
        font-weight: 400;
    }

    .eclipse-text h2 {
        font-size: 3rem;
        color: #000000;
        margin: 10px 0;
        font-weight: 700;
    }

    .eclipse-image-placeholder {
        width: 50%;
        height: 300px; /* 调整高度，更贴近图片比例 */
        background-color: #D3D3D3;
        display: flex;
        justify-content: center;
        align-items: center;
        color: #666666;
        font-size: 1.2rem;
        border-radius: 5px;
    }

    .click-button {
        display: inline-block;
        padding: 10px 20px;
        background-color: darkred;
        color: #FFFFFF;
        font-size: 1.2rem;
        font-weight: 700;
        text-decoration: none;
        border-radius: 25px;
        transition: all 0.3s ease;
        margin-top: 20px;
    }

    .click-button::after {
        content: "➔";
        margin-left: 10px;
    }

    .click-button:hover {
        background-color: #a30000;
        transform: scale(1.02);
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
    }

    .slider-controls {
        width: 100%;
        display: flex;
        justify-content: center;
        align-items: center;
        margin-top: 10px; /* 减少上边距，紧贴内容 */
        gap: 8px; /* 调整间距 */
    }

    .dots {
        display: flex;
        gap: 6px; /* 圆点间距更小 */
    }

    .dot {
        width: 8px; /* 圆点更小 */
        height: 8px;
        background-color: #BBBBBB;
        border-radius: 50%;
    }

    .dot.active {
        background-color: #666666;
    }

    .slide-info {
        font-size: 0.8rem; /* 页码字体更小 */
        color: #666666;
        font-weight: 400;
    }

    @media (max-width: 768px) {
        .eclipse-content {
            flex-direction: column;
            gap: 10px;
        }

        .eclipse-text {
            width: 100%;
        }

        .eclipse-text h1 {
            font-size: 1.2rem;
        }

        .eclipse-text h2 {
            font-size: 2rem;
        }

        .eclipse-image-placeholder {
            width: 100%;
            height: 200px;
            margin: 10px 0;
            font-size: 1rem;
        }

        .click-button {
            font-size: 1rem;
            padding: 8px 16px;
        }

        .slide-info {
            font-size: 0.7rem;
        }

        .dot {
            width: 6px;
            height: 6px;
        }

        .dots {
            gap: 4px;
        }
    }

        /* Guts Section */
        
        .guts-section {
            width: 100%;
            background-color: #1C2526;
            padding: 40px;
            box-sizing: border-box;
            display: flex;
            justify-content: space-between;
            align-items: center;
            color: #FFFFFF;
        }

        .guts-text {
            width: 50%;
        }

        .guts-text h1 {
            font-size: 6rem;
            color:white;
            margin: 0;
            letter-spacing: 8px; /* 增加字母间距 */
            text-transform: uppercase;
            font-weight: 400; /* 常规粗细 */
        }

        .guts-text p {
            font-size: 1.2rem;
            line-height: 1.8; /* 提高行高，可读性更好 */
            margin: 20px 0;
            font-weight: 400;
        }

        .guts-text h3 {
            font-size: 1.2rem;
            font-weight: 400;
            text-decoration: underline;
            margin: 10px 0 0;
        }

        .guts-image-placeholder {
            width: 40%;
            height: 500px;
            background-color: #D3D3D3;
            display: flex;
            justify-content: center;
            align-items: center;
            color: #666666;
            font-size: 1.2rem;
            border-radius: 5px;
        }

        /* Responsive Design */
        @media (max-width: 768px) {
            .guts-section {
                flex-direction: column;
                padding: 20px;
            }

            .guts-text {
                width: 100%;
            }

            .guts-text h1 {
                font-size: 4rem;
                letter-spacing: 5px;
            }

            .guts-text p {
                font-size: 1rem;
                line-height: 1.6;
            }

            .guts-image-placeholder {
                width: 100%;
                height: 300px;
                margin-top: 20px;
            }

            .links-container {
                width: 90%;
            }

            .button {
                width: 80%;
                font-size: 1.4rem;
                height: 60px;
                line-height: 60px;
            }

            .eclipse-section h1 {
                font-size: 1.2rem;
            }

            .eclipse-section h2 {
                font-size: 2rem;
            }

            .eclipse-image-placeholder {
                height: 200px;
                font-size: 1rem;
            }

            .click-button {
                font-size: 1rem;
                padding: 8px 16px;
            }

            .slide-info {
                font-size: 0.8rem;
            }

            .dot {
                width: 8px;
                height: 8px;
            }
        }

        @media (max-width: 768px) {
            .container {
                width: 95%;
                padding: 20px;
            }

            h1 {
                font-size: 1.2rem;
            }

            h2 {
                font-size: 2rem;
            }

            .image-placeholder {
                height: 200px;
                font-size: 1rem;
            }

            .click-button {
                font-size: 1rem;
                padding: 8px 16px;
            }

            .slide-info {
                font-size: 0.8rem;
            }

            .dot {
                width: 8px;
                height: 8px;
            }
        }

        .main {
            width: 70%;
            margin-left: 15%;
            min-height: 100vh;
            background-color: #FFFFFF;
            display: flex;
            flex-direction: column;
            /*justify-content: center;*/

        }

        .img {
            width: 100%;
            position: relative;
        }

        .img-view {
            width: 100%;
            height: 100%;
        }
        .tab1 {
        width: 100%;
        height: 200px;
        background-color: #546c74;
        color: #FFFFFF;
        text-align: center;
        line-height: 200px;
        font-size: 1.6rem;
        font-weight: bold;
        transition: all 0.3s ease; /* 添加平滑过渡效果 */
    }

    .tab1:hover {
        background-color: #3e5056; /* 悬浮时背景颜色变深 */
        transform: scale(1.02); /* 轻微放大 */
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); /* 添加阴影 */
    }

    .tab1 a:hover {
        color: #e0e0e0; /* 悬浮时文字颜色略变浅 */
    }

        .text1{
            position: absolute;
            z-index: 100;
            width: 100%;
            top: 300px;
            text-align: center;
            font-size: 10rem;

        }
        .text2{
            position: absolute;
            z-index: 100;
            width:40%;
            background-color: darkred;
            margin-left: 30%;
            border-radius: 50px;
            color: #FFFFFF;
            top: 650px;
            text-align: center;
            font-size: 5rem;
        }

        .text3{
            position: absolute;
            z-index: 100;
            width:40%;
            background-color: darkred;
            margin-left: 30%;
            border-radius: 50px;
            color: #FFFFFF;
            top: 1100px;
            text-align: center;
            font-size: 5rem;
        }

        .menu {
            width: 100%;
            position: absolute;
            z-index: 100;
            top: 0px;
            display: flex;
            flex-direction: row;
            justify-content: flex-end;
        }

        .text {
            color: #FFFFFF;
            margin-right: 40px;
            font-size: 1.8rem;
            font-weight: bold;

        }

        .canvas {
            width: 100%;
            height: 100px;
            display: flex;
            flex-direction: row;
            justify-content: center;
            position: absolute;
        }

        #eye-canvas {
            position: absolute;
            z-index: 999;
            pointer-events: none;
            top: 467px;
            left: 45%;
        }

        .eye-container {


            width: 100px;
            height: 100px;

            /*position: absolute;*/
            /*left: 45%; !* 调整水平位置 *!*/
            /*top: 38%; !* 调整垂直位置 *!*/
            /*width: 120px;*/
            /*height: 120px;*/
        }

    </style>
</head>
<body>
<!-- 你的页面其他内容 -->


<!-- 眼睛容器 -->


<div class="container">
    <div class="main">

        <div class="img">
            <div class="menu">
                <div class="text"><a href="https://www.baidu.com/" style="color: #FFFFFF;text-decoration: none"
                                     target="_blank">TOP</a></div>
                <div class="text"><a href="https://www.baidu.com/" style="color: #FFFFFF;text-decoration: none"
                                     target="_blank">product</a></div>
                <div class="text"><a href="https://www.baidu.com/" style="color: #FFFFFF;text-decoration: none"
                                     target="_blank">illustration</a></div>
                <div class="text"><a href="https://www.baidu.com/" style="color: #FFFFFF;text-decoration: none"
                                     target="_blank">Character</a></div>
            </div>
            <img class="img-view" src="https://cdn.ddyt.cc/github/3.svg" alt="">
        </div>
        <div class="tab1"><a href="https://www.youtube.com/watch?v=fLax0VRsM8M"
                             style="color: #FFFFFF;text-decoration: none" target="_blank">PLAY TRAILER</a></div>
<!---->
        <div class="img">

            <div class="text1">BUY NOW!</div>
            <div class="text2"><a href="https://berserk-anime.com/products/cd/index.html"
                             style="color: #FFFFFF;text-decoration: none" target="_blank">CLICK HERE!</a></div>
            <img class="img-view" src="https://cdn.ddyt.cc/github/1.svg" alt="">
        </div>
        <div class="img">
            <div class="canvas">
                <div class="eye-container">
                    <div id="eye-canvas"></div>
                </div>
            </div>
            <img class="img-view" src="https://cdn.ddyt.cc/github/2.svg" alt="">
            <div class="text3"><a href="https://berserk-anime.com/products/bd/index.htm"
                             style="color: #FFFFFF;text-decoration: none" target="_blank">CLICK HERE!</a></div>
        </div>
        <div class="eclipse-container">
            <div class="eclipse-content">
                <div class="eclipse-text">
                    <h1>Classic Illustrations</h1>
                    <h2>The Eclipse</h2>
                    <a href="https://example.com" class="click-button" target="_blank" aria-label="Learn more about The Eclipse">CLICK HERE!</a>
                </div>
                <div class="eclipse-image-placeholder" role="img" aria-label="Placeholder for The Eclipse Illustration">
                    <img src="https://cdn.ddyt.cc/github/7.svg" style="width: 100%;height: 100%"  alt="">
                </div>
            </div>
            <div class="slider-controls">
                <div class="dots">
                    <div class="dot active"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                    <div class="dot"></div>
                </div>
                <div class="slide-info">Slide 1 of 8</div>
            </div>
        </div>
        <!-- Guts Section -->
        <div class="guts-section">
            <div class="guts-text">
                <h1>G.U.T.S</h1>
                <p>Guts, renowned as the "Black Swordsman", is a former mercenary and branded wanderer who travels the world in a constant internal struggle between pursuing his own ends and upholding his attachments to those dear to him.</p>
                <h3>Main Character</h3>
            </div>
            <div class="guts-image-placeholder" role="img" aria-label="Placeholder for Guts Illustration">
                <img src="https://cdn.ddyt.cc/github/5.svg" style="width: 100%;height: 100%" alt="">
            </div>
        </div>
        <div class="img">
            <img class="img-view" src="https://cdn.ddyt.cc/github/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20250515021458.png" alt="">
        </div>
    </div>
</div>


</body>

<script>
    // 配置参数（根据实际需要调整）
    const eyeConfig = {
        size: 120,          // 眼睛直径
        irisColor: [248, 179, 119],  // RGB蓝色虹膜
        pupilSize: 45,      // 瞳孔直径
        trackingSpeed: 0.15  // 跟踪平滑度（0-1）
    };

    let eye;

    function setup() {
        const canvas = createCanvas(eyeConfig.size, eyeConfig.size);
        canvas.parent('eye-canvas');
        colorMode(RGB);
        angleMode(DEGREES);
        noStroke();

        // 初始化眼睛位置
        eye = {
            x: width / 2,
            y: height / 2,
            currentAngle: 0
        };
    }

    function draw() {
        clear();

        // 计算平滑跟踪
        const targetAngle = atan2(mouseY - eye.y, mouseX - eye.x);
        eye.currentAngle = lerpAngle(eye.currentAngle, targetAngle, eyeConfig.trackingSpeed);

        push();
        translate(eye.x, eye.y);

        // 绘制眼白（带立体感）
        fill(245);
        ellipse(0, 0, eyeConfig.size, eyeConfig.size);
        fill(230);
        ellipse(0, 0, eyeConfig.size - 8, eyeConfig.size - 8);

        // 绘制虹膜
        rotate(eye.currentAngle);
        fill(...eyeConfig.irisColor);
        ellipse(eyeConfig.size / 6, 0, eyeConfig.pupilSize, eyeConfig.pupilSize);

        // 添加高光
        fill(255, 180);
        ellipse(eyeConfig.size / 6 + 8, -8, 12, 12);

        pop();
    }

    // 角度插值函数
    function lerpAngle(a, b, t) {
        const diff = (b - a + 180) % 360 - 180;
        return a + diff * t;
    }
</script>

</html>
